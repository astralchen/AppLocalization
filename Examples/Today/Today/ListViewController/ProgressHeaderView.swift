...
class ProgressHeaderView: UICollectionReusableView {
    ...
    private var valueFormat: String {
        TodayLocalizationServices.shared.resolver.string("progress.percent.format", bundle: .main)
    }
    ...
    func reloadLocalizedContent() {
        accessibilityLabel = TodayLocalizationServices.shared.resolver.string(
            "progress.label",
            bundle: .main
        )
        accessibilityValue = String(format: valueFormat, Int(progress * 100.0))
